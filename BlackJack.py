...
def check_bj(hand):  # check BJ
    ...
    if sum_hand == 21:
        print(f'Black Jack! {hand.name} wins!')
        play = False
    else:
        play = True
...
def print_sum_cards(hand):  # print sum card w or w/o A in hand
    try:
        if check_a(hand)[0] == 2 and 'A' in list(hand):
            print(f'{hand.name} Total: {check_cards(hand, True)[1]} / {check_cards(hand, True)[2]}')
        else:
            print(f'{hand.name} Total: {check_cards(hand, True)[1]}')
    except ValueError:
        print(f'{hand.name} Total: {check_cards(hand, True)[1]}')
...
def hit_stand_in():  # check if player wants to hit or to stand
    while True:
        try:
            choose = input('Please choose: Hit(H) or Stand(S): '.strip()).upper()
            if choose == 'H':
                player1.next_deal()
                player_hit = True
                break
...
def check_play_again():  # check if player want to play again
    while True:
        try:
            choose = input('Do you want to play again? (Y/N): '.strip()).upper()
            if choose == 'Y':
                play_again = True
                break
